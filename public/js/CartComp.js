Vue.component('cart', {
   ...
   mounted() {
      this.$parent.$parent.$root.getJson(this.cartUrl)
         .then(data => {
   ...
   methods: {
      addProduct(product) {
         ...
         if (find) {
            this.$parent.putJson(this.cartUrl + find.id, { quantity: 1 })
               .then(data => {
                  if (!!data && data.result) {
                     find.quantity++;
                  }
      ...
      remove(product) {
         ...
         if (find && find.quantity > 1) {
            ...
            this.$parent.removeJson(this.cartUrl + find.id, find);
            this.cartItems.splice(this.cartItems.indexOf(product), 1);
         }
      },
      clearCart(){
        this.$parent.clearJson(this.$parent.removeAll);
        this.cartItems = []
      }
   },
   template: `
